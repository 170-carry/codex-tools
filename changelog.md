## 更新日志
- v1.3.0
  1. 修复同一工作区下不同账号会互相顶掉的问题，账号唯一键改为按用户身份和工作区稳定区分。
  2. 修复账号存储损坏时被重建清空的问题，改为优先从损坏文件、备份和临时文件中提取并恢复可用数据。
  3. 补齐远程部署缺少 Rust 工具链时的自动安装路径，减少 `cargo` 缺失导致的部署失败。
- v1.2.1
  1. 增加账号别名编辑，支持直接在账号卡片内重命名并持久化保存。
  2. 重做当前账号标识，将其移动到套餐标签后并调整为更醒目的毛玻璃样式。
- v1.2.0
  1. 优化设置页信息布局，增加版本信息、GitHub 仓库直达链接、问题反馈入口、版本发布与更新日志入口。
  2. 重做应用图标，调整为更贴近 macOS 风格的圆角纯白底图标。
- v1.1.3
